...
def webauto_testcase2(self):  # 流程 的 相关接口
    self.driver2 = webdriver.Firefox()
    self.driver2.get("http://www.baidu.com")
    sql = "SELECT b.id,b.webfindmethod,b.webevelement,b.weboptmethod,b.webtestdata,b.webassertdata,b.webtestresult,a.webcasename,b.webteststep from webtest_webcasestep as b,webtest_webcase as a where bwebcase_id=a.id AND b.webcase_id=2 ORDER BY id ASC "
    coon = pymysql.connect(user='root', passwd='test123456', db='autotest', port=3306, host='127.0.0.1', charset='utf8')
    cursor = coon.cursor()
    aa = cursor.execute(sql)
    info = cursor.fetchmany(aa)
...
def writeBug(bug_id, webcasename, webteststep, result, res_check):
    ...
    bugdetail = '[测试步聚]<br />' + str(webteststep) + '<br/>' + '[预期结果]<br/>' + str(
        res_check) + '<br/>' + '<br/>' + '[实际结果]<br />' + '<br/>' + str(result)
    print(bugdetail)
